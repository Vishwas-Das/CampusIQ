...
const SECTIONS: SectionDef[] = [
  ...
  { key: 'academic', label: 'Academic profile', description: 'College, branch, semester, links.', icon: GraduationCap },
  ...
  { key: 'privacy', label: 'Privacy', description: 'What other students can see about you.', icon: Shield },
  { key: 'danger', label: 'Danger zone', description: 'Permanent destructive actions.', icon: AlertTriangle },
]

// ════════════════════════════════════════════════════════════
// Page shell — sidebar tabs + panel layout
// ════════════════════════════════════════════════════════════
...
function SectionPanel({ section }: { section: SectionDef }) {
  return (
    <Card className="space-y-4">
      ...
      {section.key === 'academic' && <AcademicSection />}
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handlePasswordSubmit} className="space-y-3">
        ...
        {pwdError && (
          <p className="text-xs text-danger">{pwdError}</p>
        )}
        {pwdSaved && (
...
function AcademicSection() {
  ...
  const [semester, setSemester] = useState<string>(sp?.semester ? String(sp.semester) : '')
  const [cgpa, setCgpa] = useState<string>(sp?.cgpa ? String(sp.cgpa) : '')
  const [github, setGithub] = useState(sp?.github_url ?? '')
  const [linkedin, setLinkedin] = useState(sp?.linkedin_url ?? '')
  const [skills, setSkills] = useState<string>((sp?.skills ?? []).join(', '))
  const [targetRole, setTargetRole] = useState(sp?.target_role ?? '')
  ...
  if (user?.role !== 'student') {
    return (
      <p className="text-sm text-[var(--text-tertiary)]">
        Academic profile is for student accounts.
      </p>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Input label="Branch" placeholder="Computer Science" value={branch} onChange={(e) => setBranch(e.target.value)} />
        <Input label="Semester" type="number" min={1} max={8} value={semester} onChange={(e) => setSemester(e.target.value)} />
        <Input label="CGPA" type="number" step="0.01" min={0} max={10} value={cgpa} onChange={(e) => setCgpa(e.target.value)} />
        <Input label="Target role" placeholder="SDE, Data Scientist…" value={targetRole} onChange={(e) => setTargetRole(e.target.value)} />
        <Input label="GitHub URL" placeholder="https://github.com/…" value={github} onChange={(e) => setGithub(e.target.value)} />
        <Input label="LinkedIn URL" placeholder="https://linkedin.com/in/…" value={linkedin} onChange={(e) => setLinkedin(e.target.value)} />
      </div>
      <Input
...
interface NotificationPrefs {
  emailDigest: boolean
  emailQuizDue: boolean
  emailAnnouncements: boolean
  pushDoubts: boolean
  pushLeaderboard: boolean
  pushStreak: boolean
...
const NOTIFICATION_STORAGE_KEY = 'campusiq.notification-prefs'
  ...
  const groups: { title: string; items: { key: keyof NotificationPrefs; label: string; hint: string }[] }[] = [
    {
      title: 'Email',
      items: [
        { key: 'emailDigest', label: 'Weekly digest', hint: 'Monday morning summary of your week.' },
        { key: 'emailQuizDue', label: 'Quiz reminders', hint: 'Heads-up the day before a quiz closes.' },
        { key: 'emailAnnouncements', label: 'Teacher announcements', hint: 'When teachers post in your subjects.' },
      ],
    },
    {
      title: 'Push',
      items: [
        { key: 'pushDoubts', label: 'Doubt answers', hint: 'When someone answers a doubt you posted.' },
        { key: 'pushLeaderboard', label: 'Leaderboard moves', hint: 'When you go up or down a rank.' },
        { key: 'pushStreak', label: 'Streak reminder', hint: 'Gentle nudge before you lose your streak.' },
      ],
    },
    {
      title: 'In-app',
...
// ════════════════════════════════════════════════════════════
// 4. Appearance — wired to the shared useTheme store so the TopBar
//    cycle button and this picker stay in sync.
// ════════════════════════════════════════════════════════════
...
interface PrivacyPrefs {
  ...
  showStreak: boolean
  shareSkillsWithRecruiters: boolean
}
...
const DEFAULT_PRIVACY: PrivacyPrefs = {
  ...
  showStreak: true,
  shareSkillsWithRecruiters: false,
}

const PRIVACY_STORAGE_KEY = 'campusiq.privacy-prefs'
...
function PrivacySection() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-[var(--text-tertiary)]">
        Control what other students and recruiters can see about you.
      </p>
      <ToggleRow
        label="Public profile"
        hint="Allow your profile page to be viewed by other CampusIQ users."
        checked={prefs.publicProfile}
        onChange={(v) => update('publicProfile', v)}
      />
      <ToggleRow
        label="Show on leaderboard"
        hint="Display your name and XP on class and college leaderboards."
        checked={prefs.showOnLeaderboard}
        onChange={(v) => update('showOnLeaderboard', v)}
      />
      <ToggleRow
        label="Show study streak"
        hint="Streak count is visible on your public profile."
        checked={prefs.showStreak}
        onChange={(v) => update('showStreak', v)}
      />
      <ToggleRow
        label="Share skills with recruiters"
        hint="Companies browsing CampusIQ can see your skill list."
        checked={prefs.shareSkillsWithRecruiters}
        onChange={(v) => update('shareSkillsWithRecruiters', v)}
      />
      {saved && (
        <p className="text-xs text-success flex items-center gap-1.5">
          <Check className="h-3.5 w-3.5" /> Saved.
    ...
    <div className="space-y-4">
      <div className="p-3 rounded-md border border-danger/30 bg-danger/5 space-y-1">
        ...
        <p className="text-xs text-[var(--text-secondary)]">
          Permanently remove your CampusIQ account, your quizzes, your study history,
          and your gamification progress. This cannot be undone.
        </p>
      </div>
      ...
      >
        <div className="space-y-3">
          <p className="text-sm text-[var(--text-secondary)]">
            This will delete everything tied to your account — quizzes you took,
            doubts you posted, your resume, badges, and progress. There is no
            undo.
          </p>
          <p className="text-sm text-[var(--text-secondary)]">
