...
import { ArrowLeft, AlertCircle, Loader2, Play, Send, RotateCcw, Trophy } from 'lucide-react'
...
import type { ProblemDetail, ProblemRunnerPayload, SubmitResultResponse } from '../../types'
...
export default function CodingProblemPage() {
  ...
  const [acBanner, setAcBanner] = useState<SubmitResultResponse | null>(null)
      ...
      {acBanner && (
        <div className="mb-3 flex items-center gap-3 p-3 rounded-lg border border-success/30 bg-success/10 text-sm">
          ...
          <div className="flex-1">
            <div className="font-semibold text-success">First Accepted!</div>
            <div className="text-[var(--text-secondary)] text-[12.5px]">
              +{acBanner.xp_earned} XP
              {acBanner.leveled_up
                ? ` — you leveled up to level ${acBanner.new_level}!`
                : ''}
